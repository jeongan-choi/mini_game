...
public class Shape extends ImageIcon {
	...
	public Shape(URL imgURL, int margin, int steps, int xBoundary, int yBoundary) {
		...
		x= (int) (Math.random() * xBoundary);	// x값은 랜덤
		y= 0;	//맨 위에서 눈처럼 떨어지도록
	}
	...
	public boolean collide (Point p2) {				//p2가 new Point(player.x, player.y)
		Point p = new Point(this.x, this.y); 		//p가 new Point(s.x, s.y)
	...
	// 객체의 모션이 일어날 수 있도록 조정하기
	public void move() {
		y += steps;			//STEPS = 30;			// 그림 객체들이 한번에 움직이는 픽슬 수
	}
}
